...
philly_weather <- read.csv("https://raw.githubusercontent.com/walkabilly/Phillybikeshare/master/Phil_temp_2016.csv")
```
...
```{r}
library(dplyr)
trip_Philly <- dplyr::select(filter(city_data,city=="Philly"),by100000,dummy)
trip_Boston <- dplyr::select(filter(city_data,city=="Boston"),by100000)
trip_Washington <- dplyr::select(filter(city_data,city=="Washington"),by100000)
