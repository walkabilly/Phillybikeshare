...
post.period <- as.Date(c("2016-11-01", "2016-11-07"))

##Modelling all Phil data
Phil_dat$start_time <- as.Date(Phil_dat$start_time)

data <- zoo(cbind(Phil_dat$by100000), as.Date(Phil_dat$start_time))

impact_phil <- CausalImpact(data = data, pre.period = pre.period, post.period = post.period, model.args = list(niter=5000, nseasons=7))
